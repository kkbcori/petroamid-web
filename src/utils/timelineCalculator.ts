  ...
  const events: TimelineEvent[] = items.map(item => {
    if (item.notEligible) {
      return { id: item.id, checklistItemId: item.id, title: item.title, description: item.description,
        dueDate: null, status: 'not_eligible' as TimelineStatus, daysUntilDue: null,
        category: item.category, mandatory: item.mandatory, completed: false, officialSource: item.officialSource };
    }
    if (item.daysBeforeTravel === null) {
      return { id: item.id, checklistItemId: item.id, title: item.title, description: item.description,
        dueDate: null, status: item.completed ? 'completed' : 'anytime' as TimelineStatus, daysUntilDue: null,
        category: item.category, mandatory: item.mandatory, completed: item.completed, officialSource: item.officialSource };
    }
    ...
    return { id: item.id, checklistItemId: item.id, title: item.title, description: item.description,
      dueDate, status, daysUntilDue, category: item.category,
      mandatory: item.mandatory, completed: item.completed, officialSource: item.officialSource };
  });
...
export function applyPetProfileToChecklist(
  checklist: ChecklistItem[], dateOfBirth: string,
  microchipNumber: string | undefined, travelDate: Date,
): ChecklistItem[] {
  ...
  return checklist.map(item => {
    ...
    if (item.id.includes('age') || item.id.includes('us_age')) return { ...item, notEligible: ageInMonths < 6 };
    return item;
  });
}
...
export function calcReadinessScore(items: ChecklistItem[]): number {
  ...
  if (!mandatory.length) return eligible.length
    ? Math.round((eligible.filter(i => i.completed).length / eligible.length) * 100) : 0;
  return Math.round((mandatory.filter(i => i.completed).length / mandatory.length) * 100);
}
