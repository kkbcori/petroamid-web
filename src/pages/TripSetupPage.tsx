    ...
    <div>
      ...
      {pets.length === 0 && (
        <div style={{ background: Colors.yellowBg, color: '#92680a', padding: 14, borderRadius: 12, marginBottom: 14, fontSize: 14 }}>
          ⚠️ Add a pet first. <button onClick={() => navigate('/pets/add')} style={{ color: '#1a52a0', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>Add pet →</button>
        </div>
      )}
      ...
      <div style={{ background: Colors.navyMid, border: `1px solid ${Colors.border}`, borderRadius: 18, padding: 20, boxShadow: `0 2px 10px ${Colors.shadow}` }}>
        <FF label="Trip Name (optional)" value={tripName} onChange={setTripName} placeholder="e.g. Summer Holiday 2025" accent="#3B5BDB" />
        ...
        <FF label="Travel Date *" value={travelDate} onChange={setTravelDate} type="date" accent="#3B5BDB" />

        <button onClick={handleCreate} disabled={pets.length === 0} style={{
          width: '100%', padding: '14px', borderRadius: 14, marginTop: 8,
          background: pets.length === 0 ? Colors.border : 'linear-gradient(135deg, #1e3a8a 0%, #3B5BDB 100%)',
          color: '#fff', border: 'none', fontWeight: 700, fontSize: 16,
          cursor: pets.length === 0 ? 'not-allowed' : 'pointer',
          boxShadow: pets.length > 0 ? '0 4px 16px rgba(30,58,138,0.35)' : 'none',
      ...
      <div style={{ marginTop: 14, padding: '12px 14px', background: TRAVEL_COLOR, borderRadius: 12, fontSize: 13, color: '#1e3a8a', border: '1px solid rgba(30,58,138,0.1)' }}>
        🔒 Free plan: 1 trip with microchip item unlocked.
      </div>
    </div>
  );
}
...
function FF({ label, value, onChange, placeholder, type = 'text', accent }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string; type?: string; accent: string }) {
  return (
