      ...
      <div style={{ background: Colors.navyMid, border: `1px solid ${Colors.border}`, borderRadius: 18, padding: 20, boxShadow: `0 2px 10px ${Colors.shadow}` }}>
        ...
        <div style={{ marginBottom: 18 }}>
          ...
          <select value={origin} onChange={e => setOrigin(e.target.value)} style={{
            ...
            fontSize: 14, color: Colors.cream, cursor: 'pointer',
          }}>
            {COUNTRIES.map(c => (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
            {DESTINATIONS.map(d => (
              <button key={d.code} onClick={() => setDestination(d.code)} style={{
                ...
                border: `2px solid ${destination === d.code ? Colors.teal : Colors.border}`,
                background: destination === d.code ? Colors.tealGlow : Colors.navyLight,
                cursor: 'pointer',
              }}>
...
function FField({ label, value, onChange, placeholder, type = 'text', min }: {
