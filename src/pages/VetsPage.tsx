// ─────────────────────────────────────────────────────────────────────────────
// VetsPage — Nearby vet hospitals map
// Same location pattern as StaysPage — maps directly to RN geolocation
// ─────────────────────────────────────────────────────────────────────────────
import React, { useState } from 'react';
import { Colors } from '../utils/theme';

const BANNER_COLOR = '#EAF0F8';
...
export default function VetsPage() {
  ...
  function buildMapUrl(searchQuery: string, type: string) {
    const q = encodeURIComponent(`${type} ${searchQuery}`);
  ...
  const FILTER_TYPES = [
    { label: '🏥 Vet Clinics',    q: 'veterinary clinic'     },
    { label: '🚨 Emergency Vet',  q: 'emergency vet hospital' },
    { label: '💊 Pet Pharmacy',   q: 'pet pharmacy'           },
    { label: '🐾 Pet Hospital',   q: 'animal hospital'        },
  ];
  const [filterType, setFilterType] = useState(FILTER_TYPES[0].q);

  const mapUrl = mapReady ? buildMapUrl(query, filterType) : null;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '85vh' }}>
      ...
      <div style={{
        ...
        <h1 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 22, fontWeight: 700, color: '#1a2952', marginBottom: 4 }}>
          🏥 Nearby Vets
        </h1>
      ...
      <div style={{ background: Colors.navyMid, border: `1px solid ${Colors.border}`, borderRadius: 16, padding: 16, marginBottom: 14 }}>
        ...
        {mode === 'location' ? (
          <button onClick={handleUseLocation} disabled={loading} style={{
            ...
            background: loading ? Colors.border : 'linear-gradient(135deg, #3B5BDB, #1e3a8a)',
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            />
            <button onClick={handlePostcodeSearch} style={{ padding: '11px 16px', borderRadius: 10, border: 'none', background: '#3B5BDB', color: '#fff', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>
              Search
            </button>
          </div>
        )}
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
            {FILTER_TYPES.map(f => (
              <button key={f.q} onClick={() => { setFilterType(f.q); setMapReady(false); setTimeout(() => setMapReady(true), 100); }} style={{
                ...
                border: `1.5px solid ${filterType === f.q ? '#3B5BDB' : Colors.border}`,
                background: filterType === f.q ? 'rgba(59,91,219,0.1)' : Colors.navyLight,
                color: filterType === f.q ? '#3B5BDB' : Colors.creammid, fontWeight: filterType === f.q ? 700 : 400,
              }}>{f.label}</button>
            ))}
          </div>
        )}
      ...
      <div style={{ padding: '10px 14px', background: '#FEF2F2', borderRadius: 12, marginBottom: 14, border: '1px solid #FECACA', display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: '#B91C1C' }}>Use the "Emergency Vet" filter or call your nearest 24hr animal hospital</div>
        </div>
      </div>
      ...
      {!mapReady && !loading && (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 32, textAlign: 'center', background: Colors.navyMid, borderRadius: 16, border: `2px dashed ${Colors.border}`, minHeight: 280 }}>
          ...
          <p style={{ color: Colors.creammid, fontSize: 13, lineHeight: 1.6 }}>
            Use your location or enter a postcode to see vet clinics and emergency animal hospitals near you.
          </p>
        </div>
      )}
