...
import type { CategoryKey } from '../utils/theme';
...
export default function ChecklistPage() {
  ...
  function toggle(itemId: string) {
    if (!trip.isPremium && !FREE_CHECKLIST_IDS.includes(itemId)) {
  ...
  function handleUnlock() {
    // Web: no real payment — demo unlock
    ...
    <div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: Colors.cream }}>Readiness Score</span>
      ...
      {unlockPrompt && (
        <div style={{
          ...
          <div style={{
            background: Colors.navyMid, borderRadius: 20, padding: 28, maxWidth: 360, width: '100%',
            ...
            <p style={{ color: Colors.creammid, fontSize: 14, textAlign: 'center', marginBottom: 20, lineHeight: 1.5 }}>
              Get access to all {trip.checklist?.length} checklist items, timeline tracking, and official source links.
            </p>
      ...
      {Object.entries(groups).map(([cat, events]) => (
        <div key={cat} style={{ marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: Colors.creammid, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              {cat}
            </span>
            <span style={{ fontSize: 11, color: Colors.creammid }}>
          ...
          {events.map(ev => {
            ...
            return (
              <div key={ev.id} style={{
                ...
                {isExpanded && (
                  <div style={{
                    padding: '0 16px 14px', borderTop: `1px solid ${Colors.borderLight}`,
                    paddingTop: 12,
                  }}>
                    <p style={{ fontSize: 13, color: Colors.creammid, lineHeight: 1.6, marginBottom: 10 }}>
                      {ev.description}
                    </p>
                    {ev.dueDate && (
                    ...
                    <a href={ev.officialSource} target="_blank" rel="noopener noreferrer" style={{
...
function StatusBadge({ status, daysUntilDue, mandatory }: {
  ...
  return (
    <div style={{ display: 'flex', gap: 6, marginTop: 4, flexWrap: 'wrap' }}>
      ...
      {mandatory && (
        <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 20, background: Colors.redBg, color: Colors.red }}>
          Required
        </span>
      )}
    </div>
